...
当前公开的 `v0.2.0` 版本面向本地 Codex 工作流。存储适配覆盖普通项目和有明确治理锚点的
严格项目；历史 `.coop-loop` 配对不会自动迁移。其他宿主的支持范围必须等待
真实环境证据，不能由静态文档推导。

版本 `v0.2.0` 是当前公开软件版本标识。内部共识稿、私有项目路径、真实任务 ID、
状态文件、报告和凭证都不属于公开仓库。

## 后续方向

当前公开版本已在 GitHub Public 仓库提供。后续工作仍应保持同一原则：先明确公开
边界，再验证结构和行为，最后才讨论远端仓库动作。后续 GitHub 提交、推送、Release
